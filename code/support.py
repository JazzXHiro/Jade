...
def monster_importer(cols, rows, *path):
    monster_dict = {} #{'monster': {'attack': [], 'idle': []}}
    for folder_path, sub_folders, image_names in walk(join(*path)):
        for image in image_names:
            image_name = image.split('.')[0]
...
def draw_bar(surface, rect, value, max_value, color, bg_color, radius = 1):
    ...
    bg_rect = rect.copy()
    progress = max(0, min(rect.width,value * ratio))# to make sure the ratio doesn't get wider than the rectangle
    progress_rect = pygame.FRect(rect.topleft, (progress, rect.height)) #pos, size = (width, height)
