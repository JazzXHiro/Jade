...
class MonsterIndex:
    def __init__(self, monsters, fonts, monster_frames):
        ...
        self.max_stats['health'] = self.max_stats.pop('max_health') #to get rid of a key of a dictionary value
        self.max_stats['energy'] = self.max_stats.pop('max_energy')
        print(self.max_stats)
    ...
    def display_list(self):
        ...
        pygame.draw.rect(self.display_surface, 'grey', bg_rect,0,0,12,0,12,0)
        ...
        for index, monster in self.monsters.items():
            ...
            text_surf = self.font['regular'].render(monster.name, False, text_color)
            ...
            if item_rect.colliderect(self.main_rect):
                ...
                elif item_rect.collidepoint(self.main_rect.bottomleft + vector(1, -1)): #sometimes the bottomleft won't be detected properly so we need to offset it by a pixel
    ...
    def display_main(self, dt):
        #data
        monster = self.monsters[self.index] #currently selected monster
        
        #main bg
        rect = pygame.FRect(self.main_rect.left + self.list_width, self.main_rect.top, self.main_rect.width - self.list_width, self.main_rect.height) #(left, top, width, height)
        ...
        #name
        name_surf = self.font['bold'].render(monster.name, False, COLORS['white'])
        name_rect = name_surf.get_frect(topleft = top_rect.topleft + vector(10,10))
        self.display_surface.blit(name_surf, name_rect)
        
        #level
        level_surf = self.font['regular'].render(f'Lvl: {monster.level}', False, COLORS['white'])
        ...
        draw_bar(surface = self.display_surface, 
        ...
        #element
        element_surf = self.font['regular'].render(monster.element, False, COLORS['white'])
        ...
        hp_text = self.font['regular'].render(f'HP: {int(monster.health)}/{int(monster.get_stat('max_health'))}', False, COLORS['white'])
        ...
        ep_text = self.font['regular'].render(f'HP: {int(monster.health)}/{int(monster.get_stat('max_health'))}', False, COLORS['white'])
        ...
        stats_text_surf = self.font['regular'].render('Stats' , False, COLORS['white'])
        ...
        for index, (stat, value) in enumerate(monster_stats.items()):
            ...
            #text
            text_surf = self.font['regular'].render(stat, False, COLORS['white'])
        ...
        ability_text_surf = self.font['regular'].render('Ability' , False, COLORS['white'])
        ...
        for index, ability in enumerate(monster.get_abilities()):
            ...
            text_surf =  self.font['regular'].render(ability, False, COLORS['black'])
    ...
    def update(self, dt):
        self.input()#input
        self.display_surface.blit(self.tint_surf, (0,0))#tint the game
        self.display_list()#display the list
        self.display_main(dt)#display the main section